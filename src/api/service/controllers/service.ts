  ...
  ({ strapi }) => ({
    async create(ctx) {
      ...
      try {
        ...
        if (pricingModel === "flat") {
          ...
          if (variants.length) {
            throw new Error("Variants are not allowed for flat pricing.");
          }
        }

        if (pricingModel === "variant") {
          if (price) {
            throw new Error("Price should not be sent for variant pricing.");
          }
        }
        ...
        if (pricingModel === "variant" && variants.length) {
            ...
            await strapi
              ...
              .create({
                data: {
                  ...
                  expressDeliveryPrice: variant.expressDeliveryPrice,
                },
                transaction: trx,
              });
          }
    ...
    async find(ctx) {
      try {
        ...
        return ctx.badRequest(error?.message || "Failed to fetch services.");
      }
    },
    ...
    async findOne(ctx) {
      try {
        ...
        return ctx.badRequest(error?.message || "Failed to fetch service.");
      }
    },
    ...
    async update(ctx) {
      ...
      try {
        ...
        const finalPricingModel = pricingModel || existingService.pricingModel;
        ...
        if (finalPricingModel === "flat") {
          ...
          // ------------------------------------
            ...
            const existingPricing = existingService.service_pricings?.[0];
          ...
          // ------------------------------------
            ...
            for (const variant of variants) {
              ...
              if (variant.documentId) {
                ...
                if (!existingVariant) {
                  throw new Error(`Variant ${variant.documentId} not found.`);
                }
                ...
                const pricing = existingVariant.service_pricings?.[0];
    ...
    async delete(ctx) {
      ...
      try {
        ...
        return ctx.badRequest(error?.message || "Failed to delete service.");
      }
    },
    ...
    async getServicesWithVariants(ctx) {
      try {
        ...
        console.error("Get services with variants error:", error);

        return ctx.internalServerError("Unable to fetch services.");
      }
    },
  }),
);
