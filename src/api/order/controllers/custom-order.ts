...
export default {
  ...
  async orderServiceStats(ctx: Context) {
    try {
      ...
      return ctx.internalServerError("Unable to fetch service statistics.");
    }
  },
  ...
  async revenueTrends(ctx: Context) {
    try {
      ...
      return ctx.internalServerError("Unable to fetch revenue trends.");
    }
  },
  ...
  async allDashboardStats(ctx: Context) {
    try {
      ...
      const todayOrders = await strapi.db.query("api::order.order").count({
        where: {
          createdAt: {
            $gte: startOfToday,
            $lt: startOfTomorrow,
      ...
      // Yesterday orders
      const yesterdayOrders = await strapi.db.query("api::order.order").count({
        where: {
          createdAt: {
      ...
      const previousMonthlyStartDate = new Date(monthlyStartDate);

      previousMonthlyStartDate.setDate(previousMonthlyStartDate.getDate() - 30);

      const previousMonthlyEndDate = new Date(monthlyStartDate);
      ...
      const monthlyRevenuePercentage = calculatePercentageChange(
        monthlyRevenue,
      ...
      customerPeriodStart.setDate(customerPeriodStart.getDate() - 7);
      ...
      return ctx.internalServerError("Unable to fetch dashboard statistics.");
    }
  },
  ...
  async adminCreate(ctx: Context) {
    ...
    try {
      ...
      if (!userProfile) {
        return ctx.badRequest("Customer user profile is required.");
      }
      ...
      if (!customerProfile) {
        return ctx.badRequest("Customer user profile not found.");
      }
      ...
      if (!items || !items.length) {
        return ctx.badRequest("At least one item is required.");
      }
      ...
      for (const item of items) {
        if (!item.service) {
          return ctx.badRequest("Service is required for every order item.");
        }
      ...
      if (pickup_address) {
        pickupAddress = await strapi.documents("api::address.address").findOne({
        ...
        if (
          pickupAddress.user_profile?.documentId !== customerProfile.documentId
        ) {
      ...
      for (const item of items) {
        ...
        if (service.pricingModel === "flat") {
          ...
          if (!pricing) {
            throw new Error(`Pricing not found for service "${service.name}".`);
          }
        }
        ...
        // ===========================================
          ...
          if (!variant) {
            throw new Error(`Variant not found for service "${service.name}".`);
          }

          pricing = variant.service_pricings?.[0];

          if (!pricing) {
            throw new Error(`Pricing not found for variant "${variant.name}".`);
          }
        } else {
        ...
        if (pricing.price == null) {
          throw new Error(`Price not configured for "${service.name}".`);
        }
        ...
        const offerPrice =
          pricing.offerPrice !== null && pricing.offerPrice !== undefined
        ...
        const expressDeliveryPrice = Number(pricing.expressDeliveryPrice || 0);
        ...
        preparedOrderItems.push({
          ...
          expressDeliveryPrice: item.expressDelivery ? expressDeliveryPrice : 0,
      ...
      const normalItems = preparedOrderItems.filter(
        (item: any) => item.expressDelivery !== true,
      );

      const expressItems = preparedOrderItems.filter(
        (item: any) => item.expressDelivery === true,
      );
      ...
      const createOrderForItems = async (
        ...
        const grandTotal = Number(
          (orderSubTotal + tax + deliveryCharge - discount).toFixed(2),
        );
        ...
        let orderDeliveryDate = deliveryDate || null;

        let orderDeliveryTime = deliveryTime || null;

        // ===========================================
        // EXPRESS = PICKUP + 24 HOURS
        // ===========================================

        if (isExpressOrder && scheduleType === "pickup_delivery") {
          ...
          const yearValue = pickupDateTime.getFullYear();

          const monthValue = String(pickupDateTime.getMonth() + 1).padStart(
            2,
            "0",
          );

          const dayValue = String(pickupDateTime.getDate()).padStart(2, "0");

          const hoursValue = String(pickupDateTime.getHours()).padStart(2, "0");

          const minutesValue = String(pickupDateTime.getMinutes()).padStart(
            2,
            "0",
          );

          orderDeliveryDate = `${yearValue}-${monthValue}-${dayValue}`;

          // Strapi expects HH:mm:ss.SSS
          orderDeliveryTime = `${hoursValue}:${minutesValue}:00.000`;
        }
        ...
        const createdOrder = await strapi.documents("api::order.order").create({
          data: {
            ...
            pickup_address: pickupAddress?.documentId || null,
            delivery_address: deliveryAddress?.documentId || null,
            user_profile: customerProfile.documentId,
          },
        ...
        await strapi
          ...
          .create({
            data: {
              ...
              status_updated_by: adminUser.documentId,
              updatedByType: "admin",
            },
            transaction: trx,
          });
      ...
      const normalOrder = await createOrderForItems(normalItems, false);

      // ===============================================
      // Create Express Order
      // ===============================================

      const expressOrder = await createOrderForItems(expressItems, true);

      // ===============================================
      // Created Orders
      // ===============================================

      const createdOrders: any[] = [normalOrder, expressOrder].filter(Boolean);
      ...
      if (trx) {
        try {
          ...
          strapi.log.error("Admin order rollback failed:", rollbackError);
        }
      }

      strapi.log.error("Admin Create Order Error:", error);
