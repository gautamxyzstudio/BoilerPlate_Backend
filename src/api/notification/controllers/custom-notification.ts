...
export default {
  async markAsRead(ctx: Context) {
    try {
      ...
      if (!documentId) {
        return ctx.badRequest("Notification documentId is required.");
      }
      ...
      const existingReader = await strapi.db
        .query("api::notification-reader.notification-reader")
        .findOne({
          where: {
    ...
    try {
      ...
      existingReaders.forEach((reader) => {
        if (reader.notification) {
          readerMap.set(reader.notification.id, reader);
        }
      });
      ...
      for (const notification of notifications) {
        const existingReader = readerMap.get(notification.id);

        // Already read
        if (existingReader?.readAt) {
          continue;
        }
      ...
      if (readersToUpdate.length === 0 && notificationsToCreate.length === 0) {
        return ctx.send({
      ...
      await Promise.all(
        notificationsToCreate.map((notificationId) =>
          strapi.db
            .query("api::notification-reader.notification-reader")
            .create({
              data: {
      ...
      const markedCount = readersToUpdate.length + notificationsToCreate.length;
