 ...
 */
async function resolveUserContext(strapi: any, ctx: any, explicitProfileId?: string, explicitIdentifier?: string) {
  ...
  const userIdentifier = explicitIdentifier || (user ? user.id.toString() : undefined);
...
export default factories.createCoreController("api::coupon.coupon", ({ strapi }) => ({
  ...
  async apply(ctx) {
    try {
      ...
      if (orderAmount == null || isNaN(Number(orderAmount)) || Number(orderAmount) < 0) {
        return ctx.badRequest("Valid positive orderAmount is required.");
      }

      const { userProfileId: resolvedProfileId, userIdentifier: resolvedIdentifier } = await resolveUserContext(
        strapi,
        ctx,
        userProfileId,
        userIdentifier
  ...
  async validate(ctx) {
    try {
      const body = ctx.request.body?.data || ctx.request.body || ctx.query || {};
      const { code, orderAmount, userProfileId, userIdentifier } = body;
  ...
  async getAvailable(ctx) {
    try {
      const orderAmountParam = ctx.query.orderAmount || ctx.request.body?.orderAmount || 0;
      const orderAmount = Number(orderAmountParam);
      const userProfileId = ctx.query.userProfileId || ctx.request.body?.userProfileId;
      const userIdentifier = ctx.query.userIdentifier || ctx.request.body?.userIdentifier;
      ...
      return ctx.badRequest(error?.message || "Failed to fetch available coupons.");
    }
  },
