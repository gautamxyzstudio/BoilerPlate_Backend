...
export const initSocket = (httpServer: any, strapi: any) => {
  ...
  io.on("connection", async (socket) => {
      ...
      try {
        ...
        if (!orderDocumentId) {
          socket.emit("order-update-error", {
            message: "Order document ID is required.",
          });
        ...
        const loggedInUser = socket.data.user;
        ...
        if (
          ...
          socket.emit("order-update-error", {
            message: "Please provide an order status or driver assignment.",
          });
        ...
        if (pickupDriverDocumentId && deliveryDriverDocumentId) {
          socket.emit("order-update-error", {
            message: "Pickup and delivery drivers must be assigned separately.",
          });
        ...
        if (pickupDriverDocumentId) {
          ...
          const pickupDriver = await strapi
            .documents("api::driver-detail.driver-detail")
            .findOne({
              documentId: pickupDriverDocumentId,
            });

          if (!pickupDriver) {
            socket.emit("order-update-error", {
              message: "Pickup driver not found.",
            });
          ...
          if (order.orderStatus === "pending") {
            ...
            if (orderStatus && orderStatus !== "pickup_assigned") {
              socket.emit("order-update-error", {
                message:
            ...
            statusToUpdate = "pickup_assigned";
          }
          ...
          if (order.orderStatus === "pickup_assigned") {
            if (orderStatus && orderStatus !== "pickup_assigned") {
        ...
        if (deliveryDriverDocumentId) {
          ...
          const deliveryDriver = await strapi
            .documents("api::driver-detail.driver-detail")
            .findOne({
              documentId: deliveryDriverDocumentId,
            });
          ...
          if (order.orderStatus === "processing") {
            if (orderStatus && orderStatus !== "delivery_assigned") {
              socket.emit("order-update-error", {
                message:
            ...
            statusToUpdate = "delivery_assigned";
          }
          ...
          if (order.orderStatus === "delivery_assigned") {
            if (orderStatus && orderStatus !== "delivery_assigned") {
              statusToUpdate = orderStatus;
            } else {
              statusToUpdate = undefined;
        ...
        if (pickupDriverDocumentId) {
          driverUpdateData.pickup_driver = pickupDriverDocumentId;
        }

        if (deliveryDriverDocumentId) {
          driverUpdateData.delivery_driver = deliveryDriverDocumentId;
        }
        ...
        if (statusToUpdate && statusToUpdate !== order.orderStatus) {
          statusResult = await strapi
            .service("api::order-status-history.order-status-history")
            .updateOrderStatus(orderDocumentId, statusToUpdate, loggedInUser);
        }
        ...
        const customerOrder = {
          ...
          deliveryAt: updatedOrder.deliveryAt,
          ...
          pickup_address: updatedOrder.pickup_address
            ? {
                fullAddress: updatedOrder.pickup_address.fullAddress,
              }
            : null,

          delivery_address: updatedOrder.delivery_address
            ? {
                fullAddress: updatedOrder.delivery_address.fullAddress,
              }
            : null,

          delivery_driver: updatedOrder.delivery_driver
            ? {
                fullName: updatedOrder.delivery_driver.fullName,
                phoneNumber: updatedOrder.delivery_driver.phoneNumber,
              }
            : null,

          pickup_driver: updatedOrder.pickup_driver
            ? {
                fullName: updatedOrder.pickup_driver.fullName,
                phoneNumber: updatedOrder.pickup_driver.phoneNumber,
              }
            : null,
        ...
        io.to(`order-${orderDocumentId}`).emit("order-updated", {
          order: customerOrder,
          status: statusResult,
        });
    ...
    socket.on("mark-order-paid", async (data) => {
      try {
        const { orderDocumentId } = data || {};
        ...
        if (!orderDocumentId) {
          socket.emit("mark-order-paid-error", {
            message: "Order document ID is required.",
          });
        ...
        if (!allowedRoles.includes(userRole)) {
          socket.emit("mark-order-paid-error", {
            message: "You are not authorized to mark orders as paid.",
          });
        ...
        if (order.paymentStatus === "refunded") {
          socket.emit("mark-order-paid-error", {
            message: "Refunded order cannot be marked as paid.",
          });
        ...
        socket.emit("mark-order-paid-success", {
        ...
        console.error("Socket Mark Order Paid Error:", error);
